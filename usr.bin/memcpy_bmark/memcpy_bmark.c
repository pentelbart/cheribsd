...
int main(int argc, char * const argv[])
{
	...
	for(size_t i = 0; i<runs; i++)
	{
		...
		if(i==0){
			statcounters_dump_with_args(&end_bank,"MEMCPY","beri",bw_str,statfp,CSV_HEADER);
			rusage_dump_with_args(&end_rusage,"MEMCPY","beri",bw_str,rusagefp,CSV_HEADER);
		}
		else{
			statcounters_dump_with_args(&end_bank,"MEMCPY","beri",bw_str,statfp,CSV_NOHEADER);		
			rusage_dump_with_args(&end_rusage,"MEMCPY","beri",bw_str,rusagefp,CSV_NOHEADER);
